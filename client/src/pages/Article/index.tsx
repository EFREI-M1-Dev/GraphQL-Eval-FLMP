...
import {
	...
	useRemoveArticleMutation, useUpdateCommentMutation,
...
const ArticlePage = () => {
	...
	const [commentEdit, setCommentEdit] = useState<CommentEdit>({ edit: false, id: null })
	...
	const handleEditComment = (commentId: number) => {
		setCommentValue(listComments?.find(comment => comment.id === commentId)?.text || '')
	...
	return (
			<div>
								...
								<div className={styles.comments__content}>
									{listComments && listComments?.length > 0 ? (
											listComments?.map((comment, index) => (
													<div key={index} className={`${styles.comments__item} ${commentEdit.edit && comment.id === commentEdit.id ? (styles.comments__edit) : ''}`}>
														<img
																src={
														...
														{comment.author?.id === idUserConnected && !commentEdit.edit && (
																<div className={styles.comments__action}>
																	<button onClick={() => handleEditComment(comment.id)} title='Edit comment'>
																		<Icon name="write" color="#242424"/>
																	</button>
                ...
                <textarea placeholder='Write a comment...' value={commentValue}
													onChange={e => setCommentValue(e.target.value)}/>
									</div>
									{!commentEdit.edit && (
											<div className={styles.comments__area__submit}>
												<Button widthAuto onClick={handleCommentSubmit}>Comment</Button>
											</div>
									)}
									{commentEdit.edit && (
											<div className={styles.comments__area__submit}>
												<Button widthAuto onClick={handleEditCommentSubmit}>Update comment</Button>
											</div>
									)}
								</div>
							</div>
